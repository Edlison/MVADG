...
from clusgan.models import MultiNet_usps, MultiNet_usps_simple, MultiNet_simple
from clusgan.utils import save_model, calc_gradient_penalty, sample_z, cross_entropy
from clusgan.datasets import get_dataloader, dataset_list, get_anom_loader, get_multi_loader, get_multi_anom_loader
...
def main():
    ...
    parser.add_argument("-a", "-–anom", dest="anom", action='store_true', help="use anom dataset")
    ...
    net = MultiNet_usps_simple(view_num=2, latent_dim=latent_dim, cls_num=n_c, wass=wass_metric)
    ...
    if anom:
        dataloader = get_multi_anom_loader(batch=batch_size, shuffle=True)
    else:
        dataloader = get_multi_loader(batch=batch_size, shuffle=True)
    ...
    for epoch in tqdm(range(n_epochs)):
        for i, (mnist, usps) in tqdm(enumerate(dataloader)):
            net.Gs.train()
            net.Es.train()
            net.Gs.zero_grad()
            net.Es.zero_grad()
            net.Ds.zero_grad()
            (mnist_imgs, mnist_labels) = mnist
            (usps_imgs, usps_labels) = usps

            # real_imgs = [imgs.type(Tensor) for _ in range(2)]
            real_imgs = [mnist_imgs.type(Tensor), usps_imgs.type(Tensor)]
            ...
            zn, zc, zc_idx = sample_z(shape=mnist_imgs.shape[0], latent_dim=latent_dim, n_c=n_c)
