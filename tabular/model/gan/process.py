...
from v6.tabular.utils.utils import *
...
class GANProcess():
    ...
    def run(self):
        ...
        z = torch.Tensor(np.random.uniform(-1, 1, self.scaled_real_data.shape)).cuda()
        fake_data = self.generator(z).detach().cpu().numpy()
    ...
    def train_discrim(self, discriminator_optim, real_data):
        z = torch.Tensor(np.random.uniform(-1, 1, real_data.shape)).cuda()
        ...
        for _ in range(self.args.discrim_update_num):
            ...
            discrim_loss = criterion(discrim_output_fake, torch.ones((fake_data.shape[0], 1)).cuda()) + \
                           criterion(discrim_output_real, torch.zeros((real_data.shape[0], 1)).cuda())
    ...
    def train_generator(self, generator_optim, real_data):
        noise = torch.Tensor(np.random.uniform(-1, 1, real_data.shape)).cuda()
        criterion = torch.nn.BCELoss()
        ...
        for _ in range(self.args.generator_update_num):
            ...
            generator_loss = criterion(discrim_output_fake, torch.zeros((fake_data.shape[0], 1)).cuda())
